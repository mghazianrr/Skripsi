...
nstim1 = 32 #jumlah file stimulus 1
#nstim2 = 1 #jumlah file stimulus 2
...
def separate (ChName, start_sample, stop_sample): #definisikan fungsi pemisah per kanal
...
for x in range(len(folders)):
    ...
    for i in range (nstim1): 
        try:
            ...
            start_sample, stop_sample = (start_stop_seconds*sampling_freq) #sampel ke n mulai dan selesai
            d_sample = ((stop_sample - start_sample)/3).astype(int) #delta sampel, 1/3 jumlah sampel
    #        raw = preprocessing(raw)
            m = 0
            for a in Ch:
                for b in range (3):
                    np_sample = start_sample + d_sample #sampel selesai inteval
                    channel_data = separate(a, start_sample, np_sample)[0].flatten()
                    ChN[m].append(channel_data)
                    start_sample = np_sample
        #    print (ChN[m])
                    n = 0
                    o = 0
                    for j in ChN:
                        for f in freq_range:
                            l_freq, h_freq = f
                            psd, energy = feature_extraction(np.array(j).flatten(),
                                                             l_freq, h_freq)
...
#for x in range(len(folders)):
#    folder = folders[x]
#    for i in range (nstim2):                                                       #jumlah data berapa?
#        fname = os.path.join (folders[x], str(i+1) +'_stim2_raw.fif') #mengulang file dalam 1 folder
#        raw = mne.io.read_raw_fif(fname, preload = True)               #baca raw
#        info = raw.info                                                            #dapatkan info
#        sampling_freq = raw.info['sfreq'] #frekuensi sampling
#        start_seconds = 0 #detik mulai
