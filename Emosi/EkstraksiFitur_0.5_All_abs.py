...
def save(n):
    for i in range(len(Fp1_delta_psd)):
        ...
        with open ('fitur_emosi_all_abs.csv', mode='a', newline = '') as fitur_emosi:
...
#            with open ('nofilter.csv', mode='a', newline = '') as params:
...
#                fitur_writer.writerow(channel_data)
    #    print (ChN[m])
        ...
        for j in ChN:
            for f in freq_range:
                l_freq, h_freq = f
                psd, energy = feature_extraction(s, np.array(j).flatten(), l_freq, 
                                                 h_freq)
