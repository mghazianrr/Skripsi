...
asymmetry_var = [Fp12_delta_psd, Fp12_theta_psd, Fp12_alpha_psd, Fp12_beta_psd, #nama variabel asimetri
           Fp12_gamma_psd, F34_delta_psd, F34_theta_psd, F34_alpha_psd, 
           F34_beta_psd, F34_gamma_psd, C34_delta_psd, C34_theta_psd, 
           C34_alpha_psd, C34_beta_psd, C34_gamma_psd, P34_delta_psd, 
           P34_theta_psd, P34_alpha_psd, P34_beta_psd, P34_gamma_psd, 
           O12_delta_psd, O12_theta_psd, O12_alpha_psd, O12_beta_psd, 
           O12_gamma_psd, F78_delta_psd, F78_theta_psd, F78_alpha_psd, 
           F78_beta_psd, F78_gamma_psd, T34_delta_psd, T34_theta_psd, 
           T34_alpha_psd, T34_beta_psd, T34_gamma_psd, T56_delta_psd, 
           T56_theta_psd, T56_alpha_psd, T56_beta_psd, T56_gamma_psd]
...
nstim1 = 27 #jumlah file stimulus 1
nstim2 = 33
nstim3 = 17
nstim4 = 21
...
#definisikan fungsi pemisah per kanal
def separate (ChName): 
#    channel = raw[ChName, start_sample:stop_sample] #pisahkan per kanal
    channel = raw.get_data(picks = ChName).flatten
#    print (channel.shape)
    return channel #kembalikan channel


def feature_extraction (s, n, low, high):
    n = n[0]
#    print (len(n))
    win = 4 * sampling_freq
...
#    print(freq_res)
#    print (psd_values)
    power = simps(psd_values[idx_delta], dx = freq_res)
    total_power = simps(psd_values, dx=freq_res)
    rel_pow = power/total_power
    f_fft = np.fft.fftfreq(len(n), d = 1/100)
    df_fft = f_fft[1] - f_fft[0]
    E_welch = ((1 / (1/100) * (freq_res / df_fft) * 
                np.sum(psd_values[idx_delta])))
    E_tot = ((1 / (1/100) * (freq_res / df_fft) * 
                np.sum(psd_values)))
    E_rel = E_welch/E_tot
    
#    n.clear()
    
    return rel_pow, E_rel

def save(n):
    for i in range(len(Fp1_delta_psd)):
        Fp12_delta_psd = abs(Fp1_delta_psd[i]-Fp2_delta_psd[i])
        Fp12_theta_psd = abs(Fp1_theta_psd[i]-Fp2_theta_psd[i])
        Fp12_alpha_psd = abs(Fp1_alpha_psd[i]-Fp2_alpha_psd[i])
        Fp12_beta_psd = abs(Fp1_beta_psd[i]-Fp2_beta_psd[i])
        Fp12_gamma_psd = abs(Fp1_gamma_psd[i]-Fp2_gamma_psd[i])
        F34_delta_psd = abs(F3_delta_psd[i]-F4_delta_psd[i])
        F34_theta_psd = abs(F3_theta_psd[i]-F4_theta_psd[i])
        F34_alpha_psd = abs(F3_alpha_psd[i]-F4_alpha_psd[i])
        F34_beta_psd = abs(F3_beta_psd[i]-F4_beta_psd[i])
        F34_gamma_psd = abs(F3_gamma_psd[i]-F4_gamma_psd[i])
        C34_delta_psd = abs(C3_delta_psd[i]-C4_delta_psd[i])
        C34_theta_psd = abs(C3_theta_psd[i]-C4_theta_psd[i])
        C34_alpha_psd = abs(C3_alpha_psd[i]-C4_alpha_psd[i])
        C34_beta_psd = abs(C3_beta_psd[i]-C4_beta_psd[i])
        C34_gamma_psd = abs(C3_gamma_psd[i]-C4_gamma_psd[i])
        P34_delta_psd = abs(P3_delta_psd[i]-P4_delta_psd[i])
        P34_theta_psd = abs(P3_theta_psd[i]-P4_theta_psd[i])
        P34_alpha_psd = abs(P3_alpha_psd[i]-P4_alpha_psd[i])
        P34_beta_psd = abs(P3_beta_psd[i]-P4_beta_psd[i])
        P34_gamma_psd = abs(P3_gamma_psd[i]-P4_gamma_psd[i])
        O12_delta_psd = abs(O1_delta_psd[i]-O2_delta_psd[i])
        O12_theta_psd = abs(O1_theta_psd[i]-O2_theta_psd[i])
        O12_alpha_psd = abs(O1_alpha_psd[i]-O2_alpha_psd[i])
        O12_beta_psd = abs(O1_beta_psd[i]-O2_beta_psd[i])
        O12_gamma_psd = abs(O1_gamma_psd[i]-O2_gamma_psd[i])
        F78_delta_psd = abs(F7_delta_psd[i]-F8_delta_psd[i])
        F78_theta_psd = abs(F7_theta_psd[i]-F8_theta_psd[i])
        F78_alpha_psd = abs(F7_alpha_psd[i]-F8_alpha_psd[i])
        F78_beta_psd = abs(F7_beta_psd[i]-F8_beta_psd[i])
        F78_gamma_psd = abs(F7_gamma_psd[i]-F8_gamma_psd[i])
        T34_delta_psd = abs(T3_delta_psd[i]-T4_delta_psd[i])
        T34_theta_psd = abs(T3_theta_psd[i]-T4_theta_psd[i])
        T34_alpha_psd = abs(T3_alpha_psd[i]-T4_alpha_psd[i])
        T34_beta_psd = abs(T3_beta_psd[i]-T4_beta_psd[i])
        T34_gamma_psd = abs(T3_gamma_psd[i]-T4_gamma_psd[i])
        T56_delta_psd = abs(T5_delta_psd[i]-T6_delta_psd[i])
        T56_theta_psd = abs(T5_theta_psd[i]-T6_theta_psd[i])
        T56_alpha_psd = abs(T5_alpha_psd[i]-T6_alpha_psd[i])
        T56_beta_psd = abs(T5_beta_psd[i]-T6_beta_psd[i])
        T56_gamma_psd = abs(T5_gamma_psd[i]-T6_gamma_psd[i])
        ...
        fitur = (Fp1_delta_psd[i], Fp1_theta_psd[i], Fp1_alpha_psd[i], 
            ...
            T6_theta_e[i], T6_alpha_e[i], T6_beta_e[i], T6_gamma_e[i], 
            Fp12_delta_psd, Fp12_theta_psd, Fp12_alpha_psd, Fp12_beta_psd, 
            Fp12_gamma_psd, F34_delta_psd, F34_theta_psd, F34_alpha_psd, 
            F34_beta_psd, F34_gamma_psd, C34_delta_psd, C34_theta_psd, 
            C34_alpha_psd, C34_beta_psd, C34_gamma_psd, P34_delta_psd, 
            P34_theta_psd, P34_alpha_psd, P34_beta_psd, P34_gamma_psd, 
            ...
            F78_beta_psd, F78_gamma_psd, T34_delta_psd, T34_theta_psd, 
            T34_alpha_psd, T34_beta_psd, T34_gamma_psd, T56_delta_psd, 
            T56_theta_psd, T56_alpha_psd, T56_beta_psd, T56_gamma_psd, n)
        
        with open ('fitur_emosi_v7_100_abs_easym.csv', mode='a', newline = '') as fitur_emosi:
            fitur_writer = csv.writer(fitur_emosi, delimiter = ',')
            fitur_writer.writerow(fitur)
            
#Baca semua file dalam tiap folder
for x in range(len(folders)):
    ...
    for i in range (nstim): 
        ...
        #Buka file
        fname = os.path.join (folder, str(i+1) +'__raw.fif')
        raw = mne.io.read_raw_fif(fname, preload = True)
        
        #CAR, bisa digunakan atau tidak
        #raw.set_eeg_reference(ref_channels = 'average')
        info = raw.info 
        
        #Dapatkan frekuensi sampling
        sampling_freq = raw.info['sfreq']
        m = 0
        
        #Buka dan masukkan data tiap kanal ke variabel penampung
        for a in Ch:
            channel_data = separate(a)
            idx = 0
            for n in channel_data():
                idx = 0
                if abs(n) >= 100e-6:
                    channel_data()[idx] = None
                idx = idx + 1
        ...
        for j in ChN:
            for f in freq_range:
                l_freq, h_freq = f
                psd, energy = feature_extraction(s, j, l_freq, 
                                                 h_freq)
